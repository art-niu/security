...
#openssl rsa -in devops-sonarqube.key.encrypted -out devops-sonarqube.key

# Trusted Cert

# Create CSR
openssl req -new -newkey rsa:2048 -nodes -keyout $encryptedKeyDB -out $csrFile -config $certConfigFile -extensions 'v3_req'
openssl rsa -in ${encryptedKeyDB} -out $decryptedKeyDB

echo "Copy & Paste follwing string to create cert to your Trusted Cert Portal. Or find the CSR in file ${csrFile}."
cat  ${csrFile}
